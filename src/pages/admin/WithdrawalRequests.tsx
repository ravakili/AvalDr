...
export default function WithdrawalRequests() {
  ...
  useEffect(() => {
    api
      .get<WithdrawalRequest[]>("/admin/withdrawals/")
  ...
  const decide = async (status: "approved" | "rejected") => {
    if (!reviewId) return;
    try {
      await api.post(`/admin/withdrawals/${reviewId}/decide/`, {
        status,
        adminNote: note,
      });
      setList((arr) =>
  ...
  const quickDecide = async (id: string, status: "approved" | "rejected") => {
    try {
      await api.post(`/admin/withdrawals/${id}/decide/`, {
        status,
        adminNote: "",
      });
      setList((arr) =>
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard
          ...
          value={formatToman(totalPending)}
          delta={`${toFa(list.filter((w) => w.status === "pending").length)} درخواست`}
          trend="flat"
          tone="amber"
          icon={<IconWallet />}
        />
        <StatCard
          title="تأییدشده"
          value={formatToman(totalApproved)}
          delta="این ماه"
          trend="up"
          tone="teal"
          icon={<IconCheck />}
            ...
            <tbody className="divide-y divide-white/40">
              {filtered.map((w) => {
                ...
                return (
                  <tr
                    key={w.id}
                    className="text-ink-700 transition hover:bg-white/40"
                  >
                    ...
                    <td className="px-5 py-3 font-bold tabular text-ink-800">
                      {formatToman(w.amount)}
                    </td>
                    <td className="px-5 py-3 tabular text-ink-500" dir="ltr">
                      {w.bankInfo}
                    </td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <PrimaryButton variant="ghost" onClick={() => setReviewId(null)}>
              انصراف
            </PrimaryButton>
          </>
        }
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-primary-50/60 p-4 text-sm">
              <p className="text-ink-700">
                پزشک: <b>{reviewItem.doctorName}</b>
              </p>
              <p className="mt-1 text-ink-700">
                مبلغ:{" "}
                <b className="tabular">{formatToman(reviewItem.amount)}</b>
              </p>
